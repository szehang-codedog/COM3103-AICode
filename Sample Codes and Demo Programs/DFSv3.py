...
def dfs_search(start_state):
    path = [start_state]
    next_node = (start_state, path) #next_node = (A, [A])
    visited = []
    to_visit = []
    ...
    while not end: 
		...
		#Unpacking
		#At the beginning: next_state=A, path=[A]
        ...
        if not next_state in visited:
            ...
            if is_goal(next_state):
                ...
                for child_state in find_children(next_state): #find_children() will return a list
                    child_path = create_path(path, child_state) #A new path is created          
