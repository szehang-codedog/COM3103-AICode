...
def h1(state):
        ...
        count = 0
        for i in range(0,9):
                if (not state[i] == 0) and (not state[i] == goal_state[i]):
                        count += 1
...
def h2(state):
        ...
        for i in range(0,9):
                item = state[i] 
                if not item == 0 and (not i == goal_state.index(item)):
                        wrong_row = i // 3
                        wrong_col = i % 3

                        right_row = coordinate[item-1][0]
                        right_col = coordinate[item-1][1]

                        total += abs(wrong_row - right_row) + abs(wrong_col - right_col)
        return total
                ...
                if not next_state in visited:
                        ...
                        if is_goal(next_state):
                                ...
                                for child_state in find_children(next_state):
                                        ...
                                        to_visit.append([child_state, g+1, heuristic(next_state), child_path])                              
                if to_visit:
...
def main():                     
        start_state = [1,2,3,4,0,5,6,7,8]     # problem 1: easy
        #start_state = [5,1,3,4,6,8,7,0,2]     # problem 2: easy
        #start_state = [6,4,3,1,8,0,5,2,7]     # problem 3: medium
        #start_state = [1,7,4,2,3,8,6,0,5]     # problem 4: hard
        #start_state = [4,7,2,0,8,1,3,6,5]      # problem 5: hard
        time1 = time.time()
