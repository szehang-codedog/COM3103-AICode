...
def win(state):
    ...
    o_win = np.array([1,1,1])
    x_win = np.array([2,2,2])
    
    for c in state.T:
        
        if np.equal(c, o_win).all():
            return 1        
        if np.equal(c, x_win).all():
            return 2             
        
    for r in state:
     
        if np.equal(r, o_win).all():
            return 1

        if np.equal(r, x_win).all():
            return 2
            
    
    if (state.diagonal() == o_win).all() or (np.flip(state,1).diagonal() == o_win).all():        
        return 1
    
    elif (state.diagonal() == x_win).all()  or (np.flip(state,1).diagonal() == x_win).all():
        return 2
